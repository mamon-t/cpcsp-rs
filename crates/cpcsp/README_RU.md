...
Безопасная Rust-обёртка над [CryptoPro CSP 5.0](https://www.cryptopro.ru/products/csp) —
наиболее широко используемым криптографическим провайдером в России.
...
- **ГОСТ-криптография** — ГОСТ Р 34.10-2012, ГОСТ Р 34.11-2012 (Стрибог),
  ГОСТ Р 34.12-2015 (Магма / Кузнечик), ГОСТ 28147-89
...
- **Симметричное шифрование** — сессионные ключи ГОСТ 28147-89
...
- **Самоподписанные сертификаты** — генерация X.509
  (`CertCreateSelfSignCertificate`)
- **ASN.1 encode/decode** — типизированное кодирование/декодирование DER
  с RAII (`Decoded`)
...
| ГОСТ 28147-89 | Симметричное шифрование | `CALG_GOST28147_89` |
| ГОСТ Р 34.12-2015 Магма | Блочный шифр | `szOID_CP_GOST_R3412_2015_M` |
| ГОСТ Р 34.12-2015 Кузнечик | Блочный шифр | `szOID_CP_GOST_R3412_2015_K` |
...
let decoded: Decoded = unsafe { Asn1::decode_ex_alloc(szX509_PUBLIC_KEY_INFO, &der, 0)? };
...
│       │   ├── sign.rs             # CryptSignMessage, проверка отсоединённых подписей
│       │   ├── encrypt.rs          # CryptEncryptMessage
│       │   ├── pfx.rs              # PFXImportCertStore
│       │   ├── asn1.rs             # Типизированный DER encode/decode, Decoded
...
- **Один FFI-крейт**: `cpcsp-ffi-linux` (в будущем планируется `cpcsp-ffi-windows`)
...
- **Частично RAII**: `Drop` для `HCRYPTPROV`, `HCRYPTKEY`, `HCRYPTHASH`,
  `HCERTSTORE`, `PCCERT_CONTEXT`, `Decoded`, `PrivateKey`
- **Неединообразная обработка ошибок**: некоторые функции КриптоПро возвращают
  «успех» при фактической ошибке (особенности ГОСТ в MS CryptoAPI)

## Лицензия

Лицензировано под одной из двух лицензий на ваш выбор:
